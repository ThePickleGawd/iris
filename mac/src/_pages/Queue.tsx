...
const Queue: React.FC<QueueProps> = ({ setView }) => {
  ...
  const [currentModel, setCurrentModel] = useState<{ provider: string; model: string }>({ provider: "claude", model: "claude-3-5-sonnet-latest" })
  ...
  const handleChatSend = async () => {
    ...
    setChatInput("")
    try {
      const response = await window.electronAPI.invoke("claude-chat", chatInput)
